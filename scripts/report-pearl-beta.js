...
async function main() {
    ...
    const block = await withRetry(() => gnosisProvider.getBlock("latest"));
    const EVENTS_FILE = "events-pearl-beta.json"

    // StakingProxy address on gnosis
    const stakingTokenProxyAddress = "0xeF44Fb0842DDeF59D37f85D61A1eF492bbA6135d";
    ...
    const epochCounter = await withRetry(() => stakingTokenProxy.epochCounter());
    
    // Change this line to use availableRewards
    const availableRewards = await withRetry(() => stakingTokenProxy.availableRewards());
    const isLowBalance = parseFloat(availableRewards) < 50;

    const timeRemainingSeconds = Number(await withRetry(() => stakingTokenProxy.livenessPeriod())) - (block.timestamp - Number(await withRetry(() => stakingTokenProxy.tsCheckpoint())));
    ...
    const currentNumServices = Number((await withRetry(() => stakingTokenProxy.getServiceIds())).length);

    const currentDate = moment().format('MMMM D, YYYY HH:mm:ss');

    // Comment out the events gathering section
    /*
    ...
    const newEvents = await withRetry(() => stakingTokenProxy.queryFilter("*", lastLoggedBlock + 1, latestBlock));
    ...
    */
    ...
    const reportData = [
        ["Date", "Epoch", "Time Remaining in This Epoch", "Current Staked Services", "Available Rewards"],
        [currentDate, epochCounter, timeRemaining, currentNumServices, ethers.utils.formatEther(availableRewards)]
    ];

    if (isLowBalance) {
        reportData[0].splice(3, 0, "OLAS Balance Low");
        reportData[1].splice(3, 0, `Yes ${ethers.utils.formatEther(availableRewards)} OLAS`);
    }
    ...
    if (isLowBalance) {
        console.log(`\x1b[31mWarning: Low OLAS balance, ${ethers.utils.formatEther(availableRewards)} OLAS available\x1b[0m`);
    }
    console.log(`Current Staked Services: \x1b[36m${currentNumServices}\x1b[0m`);
    console.log(`Available Rewards: \x1b[36m${ethers.utils.formatEther(availableRewards)} OLAS\x1b[0m`);

    // Comment out the events-related console logs
    /*
    if (showPastEpochs) {
...
// Comment out the getEventDescription function as it's no longer used
/*
